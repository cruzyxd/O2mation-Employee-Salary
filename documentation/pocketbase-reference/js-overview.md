## JavaScript engine
    ...
    The prebuilt PocketBase v0.17+ executable comes with embedded ES5 JavaScript engine ([goja</a
...
```
    ...
    *
    ...
    For most parts, the JavaScript APIs are derived from <a href="/docs/go-overview">Go](https://github.com/dop251/goja) with 2 main differences:
...
Below is a list with some of the commonly used global objects that are accessible from everywhere:
    ...
    - 
        [
            `$app`
        ](/jsvm/modules/_app.html) - The current running PocketBase application instance.
    

    - 
        [
            `$apis.*`
        ](/jsvm/modules/_apis.html) - API routing helpers and middlewares.
    

    - 
        [
            `$os.*`
        ](/jsvm/modules/_os.html) - OS level primitives (deleting directories, executing shell commands, etc.).
    

    - 
        [
            `$security.*`
        ](/jsvm/modules/_security.html) - Low level helpers for creating and parsing JWTs, random string generation, AES encryption, etc.
...
## TypeScript declarations and code completion
    ...
    *(most editors either have it builtin or available as plugin)*.
    ...
    [reference triple-slash directive</a
    >
...
>         
    ...
    like
    <a href="https://rollupjs.org/" target="_blank" rel="noopener noreferrer">rollup](https://www.typescriptlang.org/docs/handbook/triple-slash-directives.html#-reference-path-),
...
##### Performance
    ...
    [benchmarks</a
    >). You can adjust the pool size manually with the `--hooksPool=50` flag (<em
...
##### Engine limitations
    ...
    (<a href="https://github.com/dop251/goja" target="_blank" rel="nopppener noreferrer">goja](https://github.com/pocketbase/benchmarks/blob/master/results/hetzner_cax11.md#go-vs-js-route-execution)):
    ...
    - 
        No concurrent execution inside a single handler (aka. no `setTimeout`/<code
            >setInterval</code
        >).
