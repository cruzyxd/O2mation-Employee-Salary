The most common task when using PocketBase as framework probably would be querying and working with your
    ...
    [
        `core.Record`
    ]({import.meta.env.PB_GODOC_URL}/core#Record)
...
## Hide/Unhide fields
    ...
    ]({import.meta.env.PB_GODOC_URL}/core#Record.Hide)
    and
    [
        `record.Unhide(fieldNames...)`
    ]({import.meta.env.PB_GODOC_URL}/core#Record.Unhide)
    methods.
...
## Programmatically expanding relations
    ...
    ]({import.meta.env.PB_GODOC_URL}/core#BaseApp.ExpandRecord)
    for single or
    [
        `app.ExpandRecords(records, expands, optFetchFunc)`
    ]({import.meta.env.PB_GODOC_URL}/core#BaseApp.ExpandRecords)
    ...
    ]({import.meta.env.PB_GODOC_URL}/core#Record.ExpandedOne)
    or
    [
        `record.ExpandedAll(relName)`
    ]({import.meta.env.PB_GODOC_URL}/core#Record.ExpandedAll) .
...
## Check if record can be accessed
    ...
    ]({import.meta.env.PB_GODOC_URL}/core#BaseApp.CanAccessRecord)
    method.
...
## Generating and validating tokens
    ...
    header
    *
        (see also [Builtin auth middlewares](/docs/go-routing/#builtin-middlewares) and
        [
            Retrieving the current auth state from a route
        ](/docs/go-routing/#retrieving-the-current-auth-state)
        )
    *
    .
...
```
    ...
    ]({import.meta.env.PB_GODOC_URL}/core#BaseApp.FindAuthRecordByToken)
