If you have tasks that need to be performed periodically, you could set up crontab-like jobs with the
    ...
    *
        ...
        [
            `cron.Cron`
        ]({import.meta.env.PB_GODOC_URL}/tools/cron#Cron) value)
    *
    .
    ...
    ]({import.meta.env.PB_GODOC_URL}/tools/cron#Cron.Add)
    or
    [
        `app.Cron().MustAdd(id, cronExpr, handler)`
    ]({import.meta.env.PB_GODOC_URL}/tools/cron#Cron.MustAdd)
...
```
    ...
    [
        `app.Cron().Remove(id)`
    ]({import.meta.env.PB_GODOC_URL}/tools/cron#Cron.Remove)
