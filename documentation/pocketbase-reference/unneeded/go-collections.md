Collections are usually managed via the Dashboard interface, but there are some situations where you may
    ...
    [DB migration](/docs/go-migrations)). You can find all available Collection related operations
    and methods in
    [
        `core.App`
    ]({import.meta.env.PB_GODOC_URL}/core#App)
    and
    [
        `core.Collection`
    ]({import.meta.env.PB_GODOC_URL}/core#Collection)
...
##### Custom collection query
    ...
    [
        `CollectionQuery()`
    ]({import.meta.env.PB_GODOC_URL}/core#CollectionQuery)
...
## Field definitions
    ...
    {#each fieldNames as field}
        - 
            [
                `core.{field}`
            ]({import.meta.env.PB_GODOC_URL}/core#{field})
