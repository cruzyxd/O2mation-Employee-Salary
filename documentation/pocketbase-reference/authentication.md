...
## Authenticate with password
        ...
        *
            (see also
            [Web API reference
            ](/docs/api-records/#auth-with-password)
            )
        *.
...
## Authenticate with OTP
        ...
        *
            (see also
            [Web API reference ](/docs/api-records/#auth-with-otp)
            )
        *.
...
PocketBase v0.23+ introduced optional Multi-factor authentication (MFA).
    ...
    <ol>
        ...
        - 
            ...
            **but adds the `mfaId` from the previous step as body or query parameter</strong
            >.
...
## Users impersonation
        ...
        [Impersonate endpoint](/docs/api-records#impersonate)
        .
...
## API keys
        ...
        *Dashboard > Collections > _superusers > {`{select superuser}`} > "Impersonate" dropdown option</em
        >:
...
## Auth token verification
    ...
    [Auth refresh](/docs/api-records/#auth-refresh)
...
Otherwise - returns an error response.
    ...
    <em>
        ...
        [
            benchmarks
        ](https://github.com/pocketbase/benchmarks/blob/master/results/hetzner_cax11.md#user-auth-refresh))
    *.